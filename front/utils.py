...
def add_car(unique_model_keys, unique_fuels, unique_paint_colors, unique_car_types):
    ...
    mileage = st.number_input('Mileage', min_value=0, max_value=1_000_000, value=50_000, step=1, key=f'mileage_{len(st.session_state.car_list)}')
    engine_power = st.number_input('Engine Power', min_value=0, max_value=1_000, value=100, step=1, key=f'engine_power_{len(st.session_state.car_list)}')
